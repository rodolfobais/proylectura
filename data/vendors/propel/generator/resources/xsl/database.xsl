...
<xsl:stylesheet version="1.0"
	...
	<!-- Matches root database node, the only allowed root node Starts the normalization 
		process -->
	...
	<xsl:template match='@defaultPhPNamingMethod'>
		<xsl:attribute name='defaultPhPNamingMethod'><xsl:value-of
			select='translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz")' /></xsl:attribute>
	</xsl:template>

	<!-- Normalizes any onDelete attribute by making it lowercase, or none if 
		it is empty (makes onDelete='' act the same as onDelete='none') -->
	<xsl:template match='@onDelete' name='onDelete'>
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:attribute name='onDelete'><xsl:value-of
					select='translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz")' /></xsl:attribute>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<!-- Normalizes any onUpdate attribute by making it lowercase, or none if 
		it is empty (similar to onDelete) -->
	<xsl:template match='@onUpdate' name='onUpdate'>
		<xsl:choose>
			...
			<xsl:otherwise>
				<xsl:attribute name='onUpdate'><xsl:value-of
					select='translate(., "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz")' /></xsl:attribute>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	...
	<!-- Normalize a table, add some attribute with default values if ommitted 
		and normalize all attribute and childnodes -->
	...
	<!-- Normalize a foreign-key, add some attribute with default values if 
		ommitted and normalize all attribute and childnodes -->
	...
	<!-- Just copy the unique-column node with attributes and add the vendor 
		node -->
	...
	<!-- Just copy the index-column node with attributes and add the vendor 
		node -->
	...
	<!-- Normalize a column node, add default values for missing attributes 
	...
	<xsl:template match='column'>
		<column>
			...
			<xsl:if
				test='@type = "VARCHAR" and not(boolean(@sqlType)) and not(boolean(@size))'>
